"""Forensics 7: sink semantics at FULL production shape.

The diag3 small-shape test suggested: sinks=0 == no sink at all (bf16-round).
Here at the real production shape, compare the fast kernel against
G1: keys-only softmax (no sink term).
"""
...
NEG_FULL = mx.full((1, N_HEADS, L_BAND, CATTN_KV), NEG, mx.float32)
...
o_G1 = (p32 @ kv.astype(mx.float32)) / Z32
...
def pct(a, b, tag):
    ...
    print("%-38s p50 %.5f p90 %.4f p99.9 %.4f max %.2f rms/out %.6f" % (
        tag, float(sr[int(n * 0.5)].item()), float(sr[int(n * 0.9)].item()),
        float(sr[int(n * 0.999)].item()), float(sr[n - 1].item()),
        float((rms / orms).item())))


pct(o_G1, o_fast, "G1 keys-only softmax vs fast")