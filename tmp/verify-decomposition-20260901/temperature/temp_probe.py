    ...
    for _ in range(5):
        ...
        try:
            ...
            if args.temperature is not None:
                _cal_body["temperature"] = args.temperature
            out = client.post_bench_chat_completions(_cal_body)
...
def main():
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "Additive temperature override (this fork: temp_probe.py). "
            "When provided, injects 'temperature': <value> into every request "
            "body POSTed (calibration, warmup, scored). top_p/top_k/min_p are "
            "NOT touched — they keep inheriting card values, so this stays a "
            "single-variable contrast vs requests that omit temperature "
            "(which the server resolves to the card default)."
        ),
    )
    ...
    # Additive single-variable injection (temp_probe.py fork): the ONLY change
    # vs entropy_probe.py. Builds the request body; when --temperature is
    # given, adds "temperature". No other sampling field is ever added.
    def _request_body(max_tokens_val: int) -> dict:
        body = {
            "model": args.model,
    ...
    for i in range(args.warmup):
        try:
            client.post_bench_chat_completions(_request_body(args.max_tokens))
        except Exception as e:
    ...
    for i in range(args.iterations):
        try:
            out = client.post_bench_chat_completions(_request_body(args.max_tokens))
