...
|---|---|---|---|---|
| `ml-explore/mlx` | [#3596](https://github.com/ml-explore/mlx/pull/3596) | perf(metal): coalesce sub-small_size_ allocations to single cache bucket | **0d** (today) | Opened 2026-05-27. **zcbenz reviewed within hours, asked "How did you profile?"** Replied 2026-05-27 with full methodology: `MLX_LOG_NEW_BUFFER_PATH` cache-miss histogram (~70 cache-miss allocs/step, dominated by size-4 fp32 scalars), vmmap IOAccelerator region count (before: 14K-224K climbing; after: ~2,400 flat), throughput A/B (32.1 t/s both, RSS rate 770→155 KB/tok). **Awaiting zcbenz follow-up.** 12-line patch; the strongest of today's batch. |
