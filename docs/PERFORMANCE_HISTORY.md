...
### FABLE ROUND-5 REVIEW — Fix B closed, SDPA closed, Ask B gates pre-registered, serialization contract (2026-09-02)

1. FIX B: DEAD confirmed. Do NOT open prefix-keyed cache redesign — server-side cache-architecture change with real correctness risk (KV invalidation, attention masks, positional embeddings), zero benefit at c=1. File as BACKLOG item with trigger: revisit only if V4-style concurrency returns AND cache hit rate on c>1 below X%.
2. SDPA ANOMALY: CLOSED permanently for this campaign — batched-path-only, production c=1 serial. Re-open ONLY if concurrency returns, with a pre-registered batched runbook satisfying agreed_queue_len>=2 + >=2 concurrent requests. A concurrent arm now would be a boot experiment, not root cause.
...
### HARDENING ROUNDS 2-3 — CI was testing code the cluster never ran; alignment now enforced (2026-09-03)

Commits: 67bac4498 (R1), b5ee1a113 (R2), 36072f443 + 9991126f9 + ad8bdb735 + 932fbc133 + 25a7b0c79 (R3). Reports: tmp/hardening-round{1,2,3}-20260903/REPORT.md.

**ROUND 2 — the BatchPoolingCache guard never ran, and the reason was two layers deep.** Layer 1: the test lives in the mlx-lm submodule, which CI never checks out (`checkout@v4` has no `submodules:` key) and which is outside the `pytest src` scope — incidental, no opt-out anywhere. Layer 2 (not in the brief, and it INVERTED the fix): CI resolves `mlx_lm` from a git URL in `[tool.uv.sources]`, NOT the submodule, and that pin was **9 commits stale — behind the very fix the test guards**. So "add the submodule checkout" would have tested an artifact CI never imports, and mirroring alone would have shipped CI red. Correct fix = mirror into `src/` + lock bump (7a1a4e8 -> 37260bb). Now collected by the workflow's UNMODIFIED `pytest src` scope: 4 BatchPoolingCache tests + 2 new profiler-unit tests. Mutation-verified (remove the `filter()` reindex block -> exactly 2 of 4 fail, matching round 1's independent measurement on the submodule original).

**ROUND 3 — the "mlx divergence" was MY (supervisor) PARSING ERROR, caught by the PM.** I ran `git submodule status | awk '{print substr($1,2,12)}'`, but that command puts a STATUS FLAG in column 1, so `$1` was already the full SHA and my slice dropped the leading character — producing `40a416b20851`, which `git cat-file -t` rejects as a nonexistent object. A nonexistent ref makes `merge-base --is-ancestor` fail BOTH directions with both `git log A..B` empty, which mimics divergence convincingly. **Real relationship: clean fast-forward, submodule 2 commits ahead, merge-base IS the lock pin.** Authoritative side = the submodule (start_cluster.sh:1369 excludes mlx from the lock install; :1452 compiles `./mlx` from the checkout). What CI was actually missing: 2 jaccl commits (bc8750e9c, e40a416b2) of env-gated timing instrumentation, default OFF, no kernels/attention/cache — a hygiene gap, NOT the correctness gap round 2 found. The PM said so explicitly rather than letting the earlier finding's shape inflate this one.

**THE ROOT CAUSE THAT RECURS (the real value of round 3):** a plain `uv lock` does NOT advance a git-URL pin's SHA — commit 8a04cf492 rewrote only the dev-date suffix while the SHA stayed frozen. **Drift is the DEFAULT behavior**, so the fix is a guard, not vigilance.

**ALIGNMENT GUARD SHIPPED** (932fbc133): a test in `src/` (collected by CI's existing scope) that enumerates submodules dynamically from `.gitmodules` and asserts each gitlink equals its `uv.lock` pin. Mutation-verified against the hard case — skewed the **8th of 8** lock occurrences (the one a first-match parser misses): `1 of 8 uv.lock pin(s) disagree (line 1522)` -> restore -> `2 passed`, uv.lock byte-identical after. Zero `pytest.skip` calls. Methodology note: `uv run` re-validates the lock and aborts before pytest starts, so it is useless as a mutation harness — use `.venv/bin/python`.

**LAUNCH-GATE PROPOSAL DECLINED BY THE PM, correctly.** My brief assumed a pre-launch check might be worthwhile; the PM showed `start_cluster.sh` compiles BOTH packages from the working tree, so a stale lock pin provably cannot change what the cluster installs. A launch gate would add a failure mode with no protective value — untested bash in a deploy path, unverifiable without cluster time.

Also: hc_expand Metal kernel guard mirrored into scope (ad8bdb735, 0->4 collected, executes not skips; teeth confirmed via axis-permutation and x1.01-scale mutations). Dead telemetry kit deleted (36072f443, 3 files, zero callers re-verified).
...
### HARDENING ROUND 6 — PYTEST EXECUTES IN CI FOR THE FIRST TIME; ALL GUARDS GREEN (2026-09-03)

Commits: 7dc6416a8, e3210bf8f. Report: tmp/hardening-round6-20260903/REPORT.md (299 lines).

**MILESTONE: run 33755982279 — step 7 `nix flake check` = FAILURE, step 8 `Run pytest (macOS only)` = FAILURE (i.e. it EXECUTED, 96.41s).** Control run 33753144852, same step-7 failure, step 8 = SKIPPED. Clean A/B, supervisor-verified via the jobs API on both runs.

**ROOT CAUSE: GitHub applies an implicit `success()` to any `if:` lacking a status function** — so `if: runner.os == 'macOS'` behaved as `success() && runner.os == 'macOS'`, and pytest was invisible whenever any earlier step failed. Fixed to `(success() || failure()) && runner.os == 'macOS'` — deliberately NOT `always()`, which would also burn a macOS runner on cancellation. `nix flake check` still fails the job on all 3 platforms; no `continue-on-error`; typecheck config untouched; Linux still correctly skips pytest.

**GUARD RESULTS (1316 collected / 1106 passed / 1 failed / 4 skipped / 205 deselected) — ALL 4 TRACKED GUARDS PASSED, 13 tests:** test_submodule_lock_pin_alignment.py (2), test_batch_pooling_cache_overlap.py (4), test_dsv4_mtp_phase_timer_unit_metadata.py (3), test_hc_expand_kernel.py (4). Supervisor-verified in the raw CI log. **The guards from rounds 2-5 are now live for the first time since they were written.**

**RECORD CORRECTION TO MY OWN R5 CORRECTION: the profiler guard DID exist.** My `git ls-files | grep -i profiler` missed it because the file is named after the CLASS: `test_dsv4_mtp_phase_timer_unit_metadata.py` (shipped aa2e1a599, ancestor of HEAD). Round 2's claim was accurate; my refutation was the error — a grep-term assumption, the same class of mistake as the submodule SHA mis-parse and the `gh` wrong-repo query. The PM closed its one real gap (default-unit backward-compat, e3210bf8f) and independently reproduced the mutation: sabotaging dsv4_mtp.py:828 reproduces the original bug verbatim (`rb_pool_restores mean=  3.00ms`); restore byte-identical; 3 passed.
