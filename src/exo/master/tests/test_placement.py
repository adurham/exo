...
def test_coordinator_respects_max_node_memory_limit():
    """
    Test that the coordinator respects the max_node_memory limit when placing shards.

    This simulates a scenario where a node advertises a capped memory limit via
    --max-node-memory flag. The coordinator should not assign shards that exceed
    the per-node memory threshold.
    """
    # Arrange: Create a model that requires significant memory
    # Model: 1000 KB total storage, 10 layers
    # This means ~100 KB per layer if fully loaded on one node
    model_card = ModelCard(
        model_id=ModelId("test-large-model"),
        storage_size=Memory.from_kb(1000),  # 1000 KB = ~1 MB
        n_layers=10,
        hidden_size=1000,
        supports_tensor=True,
        tasks=[ModelTask.TextGeneration],
    ...
    # Node A advertises only 100 KB available (capped by --max-node-memory)
    # Node B has plenty of memory
    node_memory = {
        node_id_a: create_node_memory(100),  # 100 KB available (capped!)
        node_id_b: create_node_memory(1000 * 1024),  # 1000 MB available
    }
    node_network = {
        node_id_a: create_node_network(),
        node_id_b: create_node_network(),
    ...
    # The model needs 1000 KB total. With 10 layers, each node would get ~5 layers
    # if evenly distributed. But node A only has 100 KB available, so it can't
    # handle 5 layers (which would need ~500 KB).
    # The coordinator should either:
    # 1. Place only on node B (single node)
    # 2. Place with uneven distribution where node A gets fewer layers
    ...
    # Act: Try to place the instance
    placements = place_instance(cic, topology, {}, node_memory, node_network)

    # Assert: Placement should succeed with appropriate shard distribution
    assert len(placements) == 1
    instance_id = list(placements.keys())[0]
    instance = placements[instance_id]

    # Verify each node's shard doesn't exceed its available memory
    for node_id, runner_id in instance.shard_assignments.node_to_runner.items():
        shard = instance.shard_assignments.runner_to_shard[runner_id]
        node_available = node_memory[node_id].ram_available

        # Calculate memory needed for this shard's layers
        n_layers = shard.end_layer - shard.start_layer
        memory_per_layer = model_card.storage_size.in_bytes // model_card.n_layers
        required_memory = memory_per_layer * n_layers

        # The shard should not require more memory than the node has available
        # (this is the key assertion for max_node_memory enforcement)
        assert required_memory <= node_available.in_bytes, (
            f"Node {node_id} assigned {n_layers} layers requiring "
            f"{required_memory} bytes but only has {node_available.in_bytes} bytes available"
        )


def test_no_valid_placement_when_max_node_memory_too_small():
    """
    Test that placement fails when max_node_memory is too small to fit any shard.

    This verifies the coordinator properly rejects placements that would exceed
    the per-node memory limit.
    """
    # Arrange: Create a model that requires significant memory
    model_card = ModelCard(
        model_id=ModelId("test-model"),
        storage_size=Memory.from_kb(1000),  # 1000 KB total
        n_layers=10,
        hidden_size=1000,
        supports_tensor=True,
        tasks=[ModelTask.TextGeneration],
    ...
    # Node advertises only 50 KB (very small limit)
    # Model needs 100 KB, so placement should fail
    node_memory = {node_id: create_node_memory(50)}  # 50 KB available
    node_network = {node_id: create_node_network()}
    ...
    # Act & Assert: Placement should fail due to insufficient memory
    with pytest.raises(ValueError, match="No cycles found with sufficient memory"):
        place_instance(cic, topology, {}, node_memory, node_network)
