...
def tensor_auto_parallel(
    ...
    elif _HAS_GEMMA4 and isinstance(model, Gemma4Model):
...
class DeepseekV4ShardingStrategy(TensorParallelShardingStrategy):
    ...
    Replicates attention on every rank; shards only the MoE block. DSv4's
    V4Attention uses a LoRA-decomposed Q/output projection plus a
    `_grouped_output_projection` that manually reshapes
    `wo_a.weight/.scales/.biases` — sharding wq_b across heads makes that
    manual reshape see half the per-group input dim and crashes inside
    `mx.quantized_matmul`. Memory-wise the attention block is ~700 MB/layer
    at 4-bit (≈30 GB across 43 layers, replicated on both ranks); the MoE
    is the bulk of the model (~130 GB), so MoE-only sharding still leaves
    comfortable headroom on 128 GB nodes (~95 GB/rank).
