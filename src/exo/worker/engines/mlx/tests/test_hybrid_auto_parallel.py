...
class TestPipelineWrapping:
    ...
    def test_tp_follower_no_pipeline_wrappers(self):
        ...
        # Last layer should NOT have a pipeline wrapper (TP follower has no sends)
        ...
        assert not isinstance(layers[-1], PipelineLastLayer)
...
class TestHybridPipelineLastLayerDataFlow:
    ...
    def _make_layer(self, *, send_to, send_during_prefill=True):
    ...
    def test_tp_master_decode_returns_local_output(self, mock_send):
        """During decode, TP master sends to PP tail, then returns local output."""
        layer, group = self._make_layer(send_to=[2])
        ...
        # Return value must be the local output, send is deferred
        assert result is local_output
        assert layer._pending_sends == [sent_output]

    @patch("mlx.core.distributed.send")
    def test_tp_follower_decode_returns_local_output(self, mock_send):
        """During decode, TP follower has no sends but returns local output."""
        layer, group = self._make_layer(send_to=[])
        layer.is_prefill = False
        ...
        # Return value must be local output
        assert result is local_output

    @patch("mlx.core.distributed.send")
    def test_pp_tail_prefill_skips_sends(self, mock_send):
    ...
    @patch("mlx.core.distributed.send")
    def test_pp_tail_decode_sends_to_all_tp_nodes(self, mock_send):
        ...
        # Returns local output
        assert result is local_output
        assert layer._pending_sends == [sent_once, sent_twice]

    @patch("mlx.core.distributed.send")
    def test_tp_follower_prefill_no_recv(self, mock_send):
        """During prefill, TP follower has no sends and does not recv."""
        layer, _group = self._make_layer(
            send_to=[], send_during_prefill=True,
        )
        layer.is_prefill = True
