...
if [ "$IS_M4_1" = true ]; then
    ...
    for NODE in "${NODES[@]}"; do
        ...
        ssh "$NODE" "export DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer && export PATH=\$(dirname \$(xcrun -f metal)):\$PATH && zsh -l -c 'cd ~/repos/exo/mlx && mkdir -p build && cd build && cmake .. -DMLX_BUILD_JACCL=ON && make -j\$(sysctl -n hw.ncpu)'" || { echo "Failed to build mlx on $NODE"; exit 1; }
    ...
    for NODE in "${NODES[@]}"; do
        ...
        EXO_ENV="EXO_KV_BITS=${EXO_KV_BITS:-false} EXO_MAX_KV_SIZE=$KV_SIZE EXO_KEEP_KV_SIZE=$KV_KEEP EXO_PREFILL_STEP_SIZE=${EXO_PREFILL_STEP_SIZE:-4096} EXO_BATCH_COMPLETION_SIZE=${EXO_BATCH_COMPLETION_SIZE:-8} EXO_MLX_WIRED_LIMIT_RATIO=${EXO_MLX_WIRED_LIMIT_RATIO:-0.87} PYTHONUNBUFFERED=${PYTHONUNBUFFERED:-1}"
