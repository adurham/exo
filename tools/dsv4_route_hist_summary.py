...
def main() -> int:
    ...
    print(
        f"Cold experts (<10% mean): {cold}/{args.n_experts}   Hot (>3x mean): {hot}/{args.n_experts}"
    )

    print("\nPer-layer concentration:")
    print(
        f"  {'Layer':>5}  {'Total':>10}  {'Top-1':>7}  {'Top-8':>7}  {'Top-32':>7}  {'Cold':>5}  {'Hot':>5}"
    )
    rows = []
    for li in sorted(by_layer.keys()):
        ...
        print(
            f"  {li:5d}  {tot:10,}  {cc[0]:6.2f}%  {cc[7]:6.2f}%  {cc[31]:6.2f}%  {cld:5d}  {hot_l:5d}"
        )
        rows.append((li, cc[7]))
    ...
    for e in universal:
        print(
            f"  E{e:3d}: hot in {int(expert_layer_count[e]):2d} layers, {int(global_counts[e]):,} total routings"
        )
