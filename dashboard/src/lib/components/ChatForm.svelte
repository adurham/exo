<script lang="ts">
	...
	function handleInput() {
		...
		textareaRef.style.height =
			Math.min(textareaRef.scrollHeight, 150) + "px";
	}
...
>
	...
	>
		...
		{#if isDragOver}
			...
			>
				<div
					class="text-exo-yellow text-sm font-mono tracking-wider uppercase"
				>
					DROP FILES HERE
				</div>
			</div>
		...
		{#if showModelSelector && availableModels().length > 0}
			...
			>
				...
				{#if currentTtft !== null || currentTps !== null}
					<div
						class="flex items-center gap-4 text-xs font-mono flex-shrink-0"
					>
						{#if currentTtft !== null}
							<span class="text-exo-light-gray">
								<span class="text-white/70">TTFT</span>
								<span class="text-exo-yellow"
									>{currentTtft.toFixed(1)}ms</span
								>
							</span>
						{/if}
						{#if currentTps !== null}
		...
		<div class="flex items-start gap-2 sm:gap-3 py-3 px-3 sm:px-4">
			...
			<!-- Terminal prompt -->
			<span
				class="text-exo-yellow text-sm font-bold flex-shrink-0 leading-7"
				>▶</span
			>
			...
			<button
				...
				class="px-2.5 sm:px-4 py-1.5 sm:py-2 rounded text-xs sm:text-xs tracking-[0.1em] sm:tracking-[0.15em] uppercase font-medium transition-all duration-200 whitespace-nowrap cursor-pointer z-20
					...
					: loading
						? 'bg-red-500/80 text-white hover:bg-red-600 hover:shadow-[0_0_20px_rgba(239,68,68,0.3)]'
